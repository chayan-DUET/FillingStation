...
<div id="breadcrumbBar" class="breadcrumb site_nav_links no_bdr_rad clearfix">
    ...
    <div class="col-md-3 col-sm-3 col-xs-4 cxs_12 no_pad">
        <ul class="text-right no_mrgn">
            <li><a href="{{ url('/home') }}"><i class="fa fa-fw fa-dashboard"></i> Dashboard</a> <span class="divider">/</span></li><li><span><i class="fa fa-file"></i> Challan</span></li>
        </ul>                            
    </div>
</div>
@if( is_Admin() )
...
<div class="form-group" style="margin-bottom:7%;">
    ...
    <div class="col-md-4">
<div class="form-group" style="margin-bottom:7%;">
    <label for="pay">Pay Order No</label>
        <select class="form-control" id="orderlist">
            <option>Select Pay Order No</option>
        </select>
    </div> 
</div>

<div id="challan" style="margin-top: 5px;">
</div>
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $(document).on("change", "#orderlist", function () {
            var payorder = $(this).val();
            var id = $('#InstituteList').val();
            $.ajax({
                url: "{{ URL::to('payorder/payorder') }}",
                type: "post",
                data: {'institute': id,'payorder':payorder, '_token': '{{ csrf_token() }}'},
                success: function (data) {
                    $('#challan').html(data);
                },
                error: function (xhr, status) {
