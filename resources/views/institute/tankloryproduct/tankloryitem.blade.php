        ...
        <tbody>
            <tr>
                ...
                <td class="text-right wmd_30" style="width:20%">
                    <a href="{{ url ('/purchaseorder/create') }}"><button class="btn btn-success btn-xs"><i class="fa fa-plus"></i> New</button></a>
            ...
            <table class="table table-bordered tbl_thin" id="check">
                <tbody>
                    <tr class="bg_gray" id="r_checkAll">
                        ...
                        <th class="text-center">Per Unit Amount(TK)</th>
                    ...
                    <tr>
                        <td>{{ $counter }}</td>
                        <td class="{{ show_hide() }} text-center">{{ $data->request_date }}</td>
                        <td class="{{ show_hide() }} text-center">{{ $data->institute->name }}</td>
                        <td class="{{ show_hide() }} text-center">{{ $data->category->category_name }}</td>
                        <td class="{{ show_hide() }} ">{{ $data->purchaseorder->order_no }}</td>
                        <td class="{{ show_hide() }} text-center">{{ $data->product->product_name }}</td>
                        <td class="text-center">{{ $data->payorder_no }}</td>
                        <td class="text-center">{{ $data->bank_description }}</td>
                        <td class="text-right">{{ $data->per_liter_amount }}</td>
                        <td class="text-right">{{ $data->order_quantity }}</td>
                        <td class="text-right">{{ $data->order_amount }}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
...
<script type="text/javascript">
    $(document).ready(function () {
        $("#search").click(function () {
            var _url = "{{ URL::to('purchaseitem/search') }}";
            var _form = $("#frmSearch");
        ...
        $("#Del_btn").click(function () {
            var _url = "{{ URL::to('purchaseitem/delete') }}";
            var _form = $("#frmList");
